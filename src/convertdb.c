...
{
  	int status = CONVERTDB_SUCCESS;
  	dbfmt_t dst_fmt = DBFMT_UNKNOWN;
...
{
	pkg_db *src = NULL, *dst = NULL;
  	int delete_src = 0, result = 0;
  	char *src_filename = NULL, *src_backup_filename = NULL, *dst_filename = NULL;
  	int status = CONVERTDB_SUCCESS, src_backup_filename_len = 0;
       ...
       ) 
	{
    	...
    	if ( src ) 
		{
	  	...
	  	/* First, try to create and open the destination format */
	  				...
	  				if ( dst_fmt == DBFMT_TEXT ) dst_filename =  concatenate_paths( get_pkg(), PKGDB_TEXT_FILE_NAME );
				#ifdef DB_BDB
	  				else if ( dst_fmt = DBFMT_BDB ) dst_filename =  concatenate_paths( get_pkg(), PKGDB_BDB_FILE_NAME );
				#endif /* DB_BDB */
	    ...
	    /* Remove any existing file */
						...
						{
	      					if ( dst_fmt == DBFMT_TEXT ) dst = create_pkg_db_text_file( dst_filename );
						#ifdef DB_BDB
	      					else if ( dst_fmt == DBFMT_BDB ) dst = create_pkg_db_bdb( dst_filename );
						#endif /* DB_BDB */
	      					else status = CONVERTDB_ERROR;
	      					if ( !dst ) 
							{
								fprintf( stderr, "Unable to create destination database at %s\n", dst_filename );
								status = CONVERTDB_ERROR;
	      					}
	    				}
	    				else 
						{
	      					fprintf( stderr, "Unable to remove existing file or directory at %s\n", dst_filename );
	      					status = CONVERTDB_ERROR;
	    				}
	  				}
	     ...
	     */
						...
						{
	      					snprintf( src_backup_filename, src_backup_filename_len, "%s.bak", src_filename );
	    ...
	    */
	    ...
	    /* Delete source or destination depending on status at finish */
				...
				else fprintf( stderr, "Database already in requested format\n" );
      		}
     		else fprintf( stderr, "Unable to allocate memory\n" );
      		close_pkg_db( src );

      		if ( src_filename ) 
			{
				if ( delete_src ) unlink( src_filename );
				
				free( src_filename );
				src_filename = NULL;
      		}
    	}
    	else fprintf( stderr, "Unable to open package database\n" );
  	}
  	else fprintf( stderr, "Internal error\n" );
}
...
{
	int status = CONVERTDB_SUCCESS, result = 0, count = 0;
  	void *n = NULL, *temp = NULL;
  	...
  	if ( src && dst ) 
  	{
		...
		/* Insert it into the destination */
				...
				{
	  				fprintf( stderr,"Error inserting %s/%s into destination database\n", key, value );
	  				status = CONVERTDB_ERROR;
				}
      		}
    	...
    	if ( status == CONVERTDB_SUCCESS ) printf( "Copied %d key/value pairs into destination database\n", count );
  	}
  	else status = CONVERTDB_ERROR;
