...
static convert_opts * alloc_convert_opts( void ) 
{
  	convert_opts *opts = malloc( sizeof( *opts ) );
  	if ( opts ) 
  	{
...
static int convert_descr( pkg_handle *ipkg, emit_pkg_streams *opkg ) 
{
  	...
  	if ( ipkg && ipkg->descr && ipkg->descr_file && opkg && opkg->pkg_tw ) 
  	{
    	...
    	if ( emit_file( ipkg->descr_file, &ti, opkg->pkg_tw ) != EMIT_SUCCESS ) 
    	{
...
{
  	int status = CONVERT_SUCCESS, result, i;
  	pkg_descr_entry *e;
  	tar_file_info ti;
 	char *src_filename = NULL;
  	...
  	if ( ipkg && ipkg->descr && opkg && opkg->emit_tw ) 
  	{
   		...
   		for ( i = 0; i < ipkg->descr->num_entries; ++i ) 
    	{
      		...
      		{
	      		if ( temp = concatenate_paths( ipkg->unpacked_dir, "package-content" ) ) 
        		{
		...
		/* Strip off leading slashes */
	        		...
	        		strncpy( ti.filename, tar_filename, TAR_FILENAME_LEN + TAR_PREFIX_LEN + 1 );
  ...
  if ( argc >= 2 ) {
    ...
    if ( opts ) {
      ...
      while ( i < argc && status == CONVERT_SUCCESS ) 
      {
	      ...
	      else 
        {
	        ...
	        else if ( strcmp( argv[i], "--set-version" ) == 0 ) 
          {
	          if ( i + 1 < argc ) 
            {
	            if ( result = set_version_arg( opts, argv[i+1] ) != CONVERT_SUCCESS ) status = result;
	        ...
	        else if ( strcmp( argv[i], "--set-compression" ) == 0 ) 
          {
	          if ( i + 1 < argc ) 
            {
	            if ( result = set_compression_arg( opts, argv[i+1] ) != CONVERT_SUCCESS ) status = result;
	            /* Consume this and the extra arg */
	            i += 2;
	          }
	          else 
      ...
      if ( status == CONVERT_SUCCESS ) 
      {
	      if ( i + 2 == argc ) 
        {
	        if ( result = set_input_file( opts, argv[i] ) == CONVERT_SUCCESS ) 
          {
	          if ( result =set_output_file( opts, argv[i+1] ) != CONVERT_SUCCESS ) status = result;
	        }
	        else status = result;
	      }
	      else if ( i + 2 < argc ) 
	      ...
	      else 
        {
          ...
          {
	          fprintf( stderr, "Syntax error: need input and output filenames\n" );
	        }
	        status = CONVERT_ERROR;
	      }
      }
...
{
  int status = CONVERT_SUCCESS, result;
  pkg_handle *ipkg;
  ...
  if ( opts && opts->emit && opts->emit->output_file && opts->input_file) 
  {
    ...
    if ( ipkg ) 
    {
      ...
      if ( opts->emit->compression == DEFAULT_COMPRESSION ) opts->emit->compression = ipkg->compression;

      if ( opts->emit->version == DEFAULT_VERSION )	opts->emit->version = ipkg->version;
...
static int set_compression_arg( convert_opts *opts, const char *arg ) 
{
  ...
  if ( opts && opts->emit && arg ) 
  {
    if ( opts->emit->compression == DEFAULT_COMPRESSION ) 
    {
      ...
      {
	      fprintf( stderr, "Unknown or unsupported compression type %s\n", arg );
	      result = CONVERT_ERROR;
      }
    }
    else 
    {
      fprintf( stderr, "Only one --set-compression option is permitted.\n" );
      result = CONVERT_ERROR;
    }
  }
...
static int set_version_arg( convert_opts *opts, const char *arg ) 
{
  ...
  if ( opts && opts->emit && arg ) 
  {
    if ( opts->emit->version == DEFAULT_VERSION ) 
    {
      ...
      {
	      fprintf( stderr, "Unknown or unsupported version %s\n", arg );
	      result = CONVERT_ERROR;
      }
    }
    else 
    {
      fprintf( stderr, "Only one --set-version option is permitted.\n" );
      result = CONVERT_ERROR;
    }
  }
  else result = CONVERT_ERROR;
