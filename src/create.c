...
static int build_pkg_descr_dirs( create_opts *, create_pkg_info *, pkg_descr * );
static int build_pkg_descr_files( create_opts *, create_pkg_info *, pkg_descr * );
static int build_pkg_descr_symlinks( create_opts *, create_pkg_info *, pkg_descr * );
...
static int scan_directory_tree_internal( create_opts *, create_pkg_info *, const char *, const char * );
...
static create_opts * alloc_create_opts( void ) 
{
	create_opts *opts = malloc( sizeof( *opts ) );
...
static int build_pkg_descr_dirs( create_opts *opts, create_pkg_info *pkginfo, pkg_descr *descr ) 
{
  	int status = CREATE_SUCCESS, result, i, count;
  	rbtree_node *n;
  	...
  	if ( opts && pkginfo && descr ) 
	{
		...
		/* count how many entries we see */
				...
				do 
				{
	  				...
	  				if ( path = rbtree_enum( pkginfo->dirs, n, &info_v, &n ) ) 
					{
	    				if ( info_v ) 
						{
	    ...
	    */
							...
							{
								if ( result = check_path_for_descr( path ) == 1 ) 
								{
		  							fprintf( stderr, "Can't use path %s in V1 format\n", path );
		 							status = CREATE_ERROR;
								}
								else if ( result == -1 ) 
								{
		...
		/* don't do more than dirs_count */
								if ( count < pkginfo->dirs_count ) 
								{
		 							...
		 							if ( i < descr->num_entries_alloced ) 
									{
										...
										{
		      								if ( descr->entries[i].filename ) free( descr->entries[i].filename );
		      								
											if ( descr->entries[i].owner ) free( descr->entries[i].owner );
		      								
											if ( descr->entries[i].group ) free( descr->entries[i].group );
...
static int build_pkg_descr_files( create_opts *opts, create_pkg_info *pkginfo, pkg_descr *descr ) 
{
  	int status = CREATE_SUCCESS, result = 0, i = 0, count = 0;
  	rbtree_node *n = NULL;
  	char *path = NULL;
  	void *info_v = NULL;
  	create_file_info *fi = NULL;
  	...
  	if ( opts && pkginfo && descr ) 
	{
		...
		/* count how many entries we see */
				...
				do 
				{
	  				...
	  				if ( path = rbtree_enum( pkginfo->files, n, &info_v, &n ) ) 
					{
	    				if ( info_v ) 
						{
		...
		/* don't do more than files_count */
								if ( count < pkginfo->files_count ) 
								{
		  							...
		  							if ( i < descr->num_entries_alloced ) 
									{
		    							...
		    							memcpy( descr->entries[i].u.f.hash, fi->hash, sizeof( fi->hash ) );
									...
									{
		    							fprintf( stderr, "Internal error during package creation (ran out of pkg_descr_entry slots at file %s)\n", path );
		    							status = CREATE_ERROR;
		  							}
								}
								else 
								{
		  							fprintf( stderr, "Internal error during package creation (saw too many files in rbtree at file %s)\n", path );
		  							status = CREATE_ERROR;
								}
	      					}
	    /*
	    ...
	    */
						...
						{
	    	  				fprintf( stderr, "Internal error during package creation (enumerating files rbtree, path %s)\n", path );
		      				status = CREATE_ERROR;
		    			}
	  				}
	  	/* else we're done */
...
static int build_pkg_descr_symlinks( create_opts *opts, create_pkg_info *pkginfo, pkg_descr *descr ) 
{
 	int status = CREATE_SUCCESS, result = 0, i = 0, count = 0;
  	rbtree_node *n = NULL;
  	char *path = NULL;
  	void *info_v = NULL;
  	create_symlink_info *si = NULL;
  	...
  	if ( opts && pkginfo && descr ) 
	{
		...
		/* count how many entries we see */
				do 
				{
	  				if ( path = rbtree_enum( pkginfo->symlinks, n, &info_v, &n ) ) 
	  				{
	    				if ( info_v ) 
						{
	    ...
	    */
		...
		/* don't do more than dirs_count */
								...
								{
		  							if ( i = descr->num_entries < descr->num_entries_alloced ) 
									{
		    							...
		    							else 
										{
											...
											if ( descr->entries[i].u.s.target ) free( descr->entries[i].u.s.target );
									...
									{
		    							fprintf( stderr, "Internal error during package creation (ran out of pkg_descr_entry slots at symlink %s)\n", path );
		   								status = CREATE_ERROR;
		  							}
								}
								else 
								{
		  							fprintf( stderr, "Internal error during package creation (saw too many symlinks in rbtree at symlink %s)\n", path );
		  							status = CREATE_ERROR;
								}	
	      					}
	    /*
	    ...
	    */	      
						...
						{
	     					fprintf( stderr, "Internal error during package creation (enumerating symlinks rbtree, path %s)\n", path );
	      					status = CREATE_ERROR;
	    				}
	  				}
	  /* else we're done */
...
static int build_pkg_descr( create_opts *opts, create_pkg_info *pkginfo, pkg_descr **descr_out ) 
{
  	int status = CREATE_SUCCESS, result = 0, i = 0;
  	pkg_descr *descr = NULL;
  	...
  	if ( opts && pkginfo && descr_out ) 
	{
    	...
    	if ( descr ) 
		{
      	...
      	/* Allocate enough space for everyting */
      		if ( get_dirs_enabled( opts ) )	descr->num_entries_alloced += pkginfo->dirs_count;
     	 	
			if ( get_files_enabled( opts ) ) descr->num_entries_alloced += pkginfo->files_count;
     		if ( get_symlinks_enabled( opts ) ) descr->num_entries_alloced += pkginfo->symlinks_count;

      		if ( descr->num_entries_alloced > 0 ) 
			{
				descr->entries = malloc( sizeof( *(descr->entries) ) *descr->num_entries_alloced );
				if ( descr->entries ) 
				{
	  				memset( descr->entries, sizeof( *(descr->entries) ) * descr->num_entries_alloced, 0 );
				}
				else 
				{
	  				fprintf( stderr, "Unable to allocate memory\n" );
		...
		/* Do the directories first */
				if ( pkginfo->dirs && get_dirs_enabled( opts ) && pkginfo->dirs_count > 0 ) 
					if ( result = build_pkg_descr_dirs( opts, pkginfo, descr ) != CREATE_SUCCESS ) status = result;
				
		/* else nothing to do for dirs */

		/* Now do the files */
				if ( status == CREATE_SUCCESS && pkginfo->files && get_files_enabled( opts ) && pkginfo->files_count > 0 ) 
	  				if ( result = build_pkg_descr_files( opts, pkginfo, descr ) != CREATE_SUCCESS ) status = result;
	
		/* else nothing to do for files */

		/* Do the symlinks last */
				if ( pkginfo->symlinks && get_symlinks_enabled( opts ) && pkginfo->symlinks_count > 0 ) 
	  				if ( result = build_pkg_descr_symlinks( opts, pkginfo, descr ) != CREATE_SUCCESS ) status = result;
...
{
  	int status = CREATE_SUCCESS, result = 0;
  	create_pkg_info *pkginfo = NULL;
  	emit_pkg_streams *streams =NULL;
  	pkg_descr *descr = NULL;
  	...
  	if ( opts ) 
	{
    	...
    	if ( pkginfo ) 
		{
       	...
       	*/
      		...
      		if ( result = scan_directory_tree( opts, pkginfo ) == CREATE_SUCCESS ) 
			{
		/* Construct the pkg_descr */
				if ( result = build_pkg_descr( opts, pkginfo, &descr ) != CREATE_SUCCESS ) status = result;
		...
		/* Now we can free the directory and symlink rbtrees */
				...
				if ( status == CREATE_SUCCESS ) 
				{
	  				if ( streams = start_pkg_streams( opts->emit ) ) 
					{
	    				if ( result = emit_descr( opts, descr, streams->pkg_tw ) != CREATE_SUCCESS ) 
						{
	    ...
	    /* Get ready to emit content */
	      					if ( result = start_pkg_content( opts->emit, streams ) == EMIT_SUCCESS ) 
							{
		/* Now emit the files */
								if ( result = emit_files( opts, pkginfo, streams->emit_tw ) != CREATE_SUCCESS ) 
								{
	    ...
	    /* Remove the output if we had an error with it */
					...
					{
	    				fprintf( stderr, "Unable to open output streams for %s\n", opts->emit->output_file );
	    				status = CREATE_ERROR;
	  				}	
		...
		/* Now we can free the files rbtree */
			...
			{
				fprintf( stderr, "Failed to scan directory tree %s for package build\n", opts->input_directory );
				status = result;
      		}
	...
	{
	  	int result = 0;
	  	char *temp = NULL, *cmp = NULL, *marker = NULL;
 	 	...
 	 	if ( path ) 
		{
 	   	if ( temp = copy_string( path ) ) 
			{
 	     		if ( cmp = get_path_component( temp, &marker ) && strcmp( cmp, "package-description" ) == 0 ) 
					result = 1;
...
{
  	int result;
  	create_opts *opts = alloc_create_opts();

	if ( opts ) 
	{
    	set_default_opts( opts );
    	if ( result = create_parse_options( opts, argc, argv ) == CREATE_SUCCESS ) build_pkg( opts );
    	free_create_opts( opts );
  	}
  	else 
	{
    	fprintf( stderr, "Unable to allocate memory trying to create package\n" );
  	}
}

int create_parse_options( create_opts *opts, int argc, char **argv ) 
{
 	int i = 0, status = CREATE_SUCCESS;
  	char *temp = NULL;

  	if ( opts && argc >= 0 && argv ) 
	{
    	for (int i = 0; i < argc && status == CREATE_SUCCESS; ++i ) 
		{
      		if ( *(argv[i]) == '-' ) 
			{
		/* It's an option */
	  	...
	  	/* End option parsing here, advancing i past the -- */
				...
				else if ( strcmp( argv[i], "--enable-dirs" ) == 0 ) 
				{
					...
					{
	    				fprintf( stderr,"Only one of --{disable|enable}-dirs permitted\n" );
	    				status = CREATE_ERROR;
	  				}
				}
				...
				else if ( strcmp( argv[i], "--enable-files" ) == 0 ) 
				{
					...
					{
	    				fprintf( stderr, "Only one of --{disable|enable}-files permitted\n" );
	    				status = CREATE_ERROR;
	  				}
				}
				...
				else if ( strcmp( argv[i], "--enable-symlinks" ) == 0 ) 
				{
					...
					{
	   					fprintf( stderr, "Only one of --{disable|enable}-symlinks permitted\n" );
	    				status = CREATE_ERROR;
	  				}
				}
	    ...
	    /* Consume the extra arg */
					...
					{
	    				fprintf( stderr, "The --set-compression option requires a parameter; try \'mpkg help create\'\n" );
	    			status = CREATE_ERROR;
	  				}	  
				}
	    ...
	    /* Consume the extra arg */
					...
					{
	    				fprintf( stderr, "The --set-pkg-time option requires a parameter; try \'mpkg help create\'\n" );
	   					status = CREATE_ERROR;
	  				}
				}
	    ...
	    /* Consume the extra arg */
					...
					{
	    				fprintf( stderr, "The --set-version option requires a parameter; try \'mpkg help create\'\n" );
	    				status = CREATE_ERROR;
	  				}
				}
				else 			
				{
	  				fprintf(stderr, "Unknown option for create %s; try \'mpkg help create\'\n", argv[i] );
	  				status = CREATE_ERROR;
				}
      		}
      /* Not an option; break out and handle the filename args */
    	...
    	if ( status == CREATE_SUCCESS ) 
		{
			...
			{
				if ( temp = get_current_dir() ) 
				{
	  				opts->input_directory = concatenate_paths( temp, argv[i] );
	  				if ( i + 2 == argc) 
					{
	    				opts->emit->output_file = concatenate_paths( temp, argv[i + 1] );
	    				opts->pkg_name = guess_pkg_name_from_output_file( opts->emit->output_file );
	    				if ( !(opts->pkg_name) ) 
						{
	      					opts->pkg_name = guess_pkg_name_from_input_directory( opts->input_directory );
	      					if ( !(opts->pkg_name) ) 
							{
								fprintf( stderr, "Unable to guess package name, and it wasn't specified in the command.\n" );
								status = CREATE_ERROR;
	      					}
	    				}
	  				}
	  				else 
					{
	  				...
	  				if ( status == CREATE_SUCCESS ) 
					{
	   					if ( !( opts->input_directory && opts->emit->output_file && opts->pkg_name ) ) 
						{
	   	...
	   	*/
     		...
     		else 
			{
				if ( i == argc ) 
				{
	  				fprintf( stderr, "Directory and output filename are required; try \'mpkg help create\'\n" );
	  				status = CREATE_ERROR;
				}
				else if ( i + 1 == argc ) 
				{
	  				fprintf( stderr, "Output filename is required; try \'mpkg help create\'\n" );
	  				status = CREATE_ERROR;
				}
				else 	
				{
	  				fprintf( stderr, "There are %d excess arguments; try \'mpkg help create\'\n",
		   			argc - ( i + 3 ) );
	  				status = CREATE_ERROR;
				}
      		}
...
{
  	create_dir_info *di = NULL, *rdi = NULL;
  	...
  	if ( v ) 
	{
    	...
    	if ( rdi ) 
		{
     		...
     		if ( !( ( rdi->owner || !(di->owner) ) && ( rdi->group || !(di->group) ) ) ) 
			{

				fprintf( stderr, "Unable to allocate memory in dir_info_copier()\n" );
