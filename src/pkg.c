...
static void help_callback( int argc, char **argv ) 
{
 	int i =0, found_it = 0;
  	...
  	if ( argc == 0 ) 
  	{
    	...
    	while ( cmd_table[i].name != NULL ) printf( "\t%s\n", cmd_table[i++].name );
    	printf( "\n" );
  	...
  	else if ( argc == 1 ) 
  	{
    	while ( cmd_table[i].name != NULL && !found_it ) 
    	{
      		if ( strcmp( cmd_table[i].name, argv[0] ) == 0 ) 
      		{
	      		...
	      		else printf( "No help available for command '%s'.\n", argv[0] );
      		}
      		++i;
    	}
  	...
  	else printf( "Wrong number of parameters; try 'mpkg help help'\n" );
}
	...
	init_pkg_globals();
  	...
  	while ( i < argc ) 
  	{
    	...
    	if ( *curr == '-' ) 
    	{
      		...
      		{
        //check if tempdir have a name of a directory after
	      		if ( i + 1 < argc ) 
				{
         			set_temp( argv[++i] );
        //the next argument is a dir name so we skip it
          			i++;
        		}
	      		else 
        		{
